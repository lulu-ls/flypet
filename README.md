...
每个物种有独立的飞行策略（Movement）、行为参数与成长档案，接入新物种只需：模型（GLB + 烘焙动画）→ 物种注册表登记 → 调参上线。

| 物种 | 状态 | 飞行风格 | 备注 |
|------|------|----------|------|
| 🦋 蝴蝶 | ✅ 已上线 | 飘忽之字 + 大角度螺旋 + 舒展滑翔 | Sketchfab 帝王蝶模型 |
| 🪰 苍蝇 | ✅ 已上线 | 急促抖拐 + 忽快忽慢（dart-pause） | Mixamo 动画，多套待机轮播 |
| 蜻蜓 | ✅ 已上线 | 笔直疾飞 + 悬停式减速 | 亲密度解锁主动亲近 |
| 🕷️ 蜘蛛 | 🚧 开发中 | 爬行（低速、几乎不挪窝） | 八足动画制作中 |
| 🐝 蜜蜂 | 🚧 规划中 | 疾飞 + 悬停采蜜 | 策略参数已占位 |
| 🦗 螳螂 | 🚧 规划中 | 静守 + 突袭 | 爱静止「埋伏」，互动性强 |
| 🪲 天牛 | 🚧 规划中 | 缓慢爬行 + 短距飞行 | 壳硬、动作憨 |

开发中的物种已注册占位（托盘菜单置灰），行为参数为占位初稿，模型就绪后即可启用。
